...
const CreateGroupDialog = (props: Props) => {
  ...
  const handleSubmit = async (
    ...
    await createGroup({ name: values.name, members: values.members })
      ...
      .catch((err) => {
        toast.error(
          err instanceof ConvexError ? err.data : "Unexpected error occored"
        );
      });
  };
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)}>
                          ...
                          {unselectedFriends.map((friend) => {
                            return (
                              ...
                              >
                                ...
                                <h4 className="trancute">{friend.username}</h4>
                              </DropdownMenuCheckboxItem>
                            );
                          })}
                        </DropdownMenuContent>
            ...
            {members && members.length ? (
              <Card className="flex items-center mt-4 gap-3 overflow-x-auto w-full h-24 p-2 no-scrollbar">
                {friends
